...
export default function EdicaoItinerario() {
  ...
  const handleDragEnd = (event) => {
    ...
    const fromIndex = itensTrajeto.findIndex(item => item.id === activeId)
    const toIndex = itensTrajeto.findIndex(item => item.id === overId)

    if (fromIndex !== -1 && toIndex !== -1) {
      handleReordenarPorDrag(fromIndex, toIndex)
    }
  }
